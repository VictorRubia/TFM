...
include an index column?'
    ...
    if len(working_data['RR_list']) > 1:
        for i in range(2, len(working_data['RR_list'])):
            rel_rr.append((2 * (working_data['RR_list'][i] - working_data['RR_list'][i-1])) / (working_data['RR_list'][i] + working_data['RR_list'][i-1]))

    measures['MEAN_REL_RR'] = mean(rel_rr)
    measures['MEDIAN_REL_RR'] = median(rel_rr)

    measures['SDRR_REL_RR'] = stdev(rel_rr)

    sum = 0

    for i in range(1, len(rel_rr)-1):
        sum += pow(rel_rr[i+1] - rel_rr[i],2)
    measures['RMSSD_REL_RR'] = sqrt(sum) / (len(rel_rr) - 1)

    measures['SDRR_RMSSD_REL_RR'] = measures['SDRR_REL_RR'] / measures['RMSSD_REL_RR']
