...
document.addEventListener('turbo:load', () => {
    ...
    // console.log( $('[name^="edit-upload"]').get() );
    // $('[name^="edit-upload"]').get().forEach(function (el) {
    //     // console.log($(el).attr('name'))
    //     console.log("Entro - " + $(el).attr('name').split("-").pop())
    //     el.innerHTML = 'Hello World';
    // });

    document.querySelectorAll('[name^="edit-upload"]').forEach((element) => setupUppyEdit(element, $(element).attr('name').split("-").pop()))
})
...
function setupUppyEdit(element, index) {
    let trigger = element.querySelector(`[data-behavior-edit-${index}="uppy-trigger-edit"]`)
    let direct_upload_url = document.querySelector("meta[name='direct-upload-url']").getAttribute("content")
    let cancel = document.querySelectorAll(".close-tags-modal")
    let field_name = element.dataset.uppy
